...
class Human36mGT2D(DataInterface):
    def __init__(self, config: dict, gtData:Human36mGT):
        ...
        self.dataset = dict(np.load(config.get("Human3.6m", "2D_gt"), allow_pickle=True))
    ...
    def get_sequence(self, subject: str, action: str) -> Sequence:
        ...
        nparr = np.empty((len(camlist),len(self.keypoints[subject][action][0]),17,3))
        for cam_idx, kps in enumerate(self.keypoints[subject][action]):
            cam = camlist[cam_idx]

            kps = normalize_screen_coordinates(kps[..., :2], w=cam.res[0], h=cam.res[1])
            nparr[cam_idx] = np.stack([kps[...,0],kps[...,1],np.ones(kps.shape[:-1])],axis=2)
